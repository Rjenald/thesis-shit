...
class _TeacherModePageState extends State<TeacherModePage> {
  ...
  // ── computed ───────────────────────────────────────────────────────────────
  int    get _totalStudents => _classes.fold(0, (s, c) => s + ((c['students'] as List?)?.length ?? 0));
  int    get _activeClasses => _classes.length;
  double get _classAvg      => _sessions.isEmpty ? 0 : _sessions.map((s) => s.score).reduce((a, b) => a + b) / _sessions.length;
  double get _totalHours    => _sessions.fold(0.0, (s, r) => s + r.durationSeconds) / 3600;

  @override
  void initState() { super.initState(); _tab = widget.initialTab; _load(); }

  Future<void> _load() async {
    final cls  = await SessionStorageService.loadClasses();
    final sess = await SessionStorageService.loadSessions();
    if (mounted) setState(() { _classes = cls; _sessions = sess; _loading = false; });
  }
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 10),
            _statCard(Icons.star_outline,          '${_classAvg.toStringAsFixed(0)}%', 'Class Avg'),
            const SizedBox(width: 10),
            _statCard(Icons.access_time_rounded,  '${_totalHours.toStringAsFixed(0)}h','Total Time'),
          ]),
        ),
      ]),
    );
  ...
  Widget _statCard(IconData icon, String value, String label) => Expanded(
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.primaryCyan.withValues(alpha: 0.15)),
      ),
      child: Column(children: [
        ...
        Text(value, style: const TextStyle(color: AppColors.white, fontSize: 16,
            fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        Text(label, style: TextStyle(color: AppColors.grey.withValues(alpha: 0.7),
  ...
  // ── Tab Bar ───────────────────────────────────────────────────────────────
  static const _tabLabels = ['Overview', 'Students', 'Assignments', 'Analytics'];
        ...
        return Expanded(
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: sel ? AppColors.primaryCyan.withValues(alpha: 0.15) : Colors.transparent,
                borderRadius: BorderRadius.circular(10),
                border: sel
                    ? Border.all(color: AppColors.primaryCyan.withValues(alpha: 0.4))
                    : null,
              ),
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Icon(_tabIcons[i],
                    color: sel ? AppColors.primaryCyan : AppColors.grey.withValues(alpha: 0.5),
                    size: 16),
  ...
  Widget _buildTabContent() {
    switch (_tab) {
      ...
      case 2: return _buildAssignmentsTab();
      case 3: return _buildAnalyticsTab();
  ...
  Widget _buildOverviewTab() {
    final sorted = List<SessionResult>.from(_sessions)..sort((a, b) => b.score.compareTo(a.score));
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      // Top performers
      _sectionHeader('Top Performers', Icons.star_rounded,
      ...
      if (_classes.isNotEmpty) ...[
        _sectionHeader('My Classes', Icons.class_outlined,
            onViewAll: () => setState(() => _tab = 2)),
        const SizedBox(height: 10),
        ..._classes.asMap().entries.map((e) => _quickClassRow(e.key, e.value)),
        const SizedBox(height: 22),
      ],
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(label, style: TextStyle(color: color, fontSize: 13,
                fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
            Text(sub, style: TextStyle(color: color.withValues(alpha: 0.65),
                fontSize: 10, fontFamily: 'Roboto')),
          ]),
        ),
      );

  Widget _sectionHeader(String title, IconData icon, {VoidCallback? onViewAll}) =>
      Row(children: [
        Icon(icon, color: AppColors.primaryCyan, size: 16),
        const SizedBox(width: 7),
        Text(title, style: const TextStyle(color: AppColors.white, fontSize: 15,
            fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        const Spacer(),
        if (onViewAll != null)
          GestureDetector(
            onTap: onViewAll,
            child: const Text('View All', style: TextStyle(color: AppColors.primaryCyan,
                fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
          ),
      ]);

  static const _rankColors = [Color(0xFFFFA726), AppColors.primaryCyan, Color(0xFF9C6FFF)];
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(s.songTitle, style: const TextStyle(color: AppColors.white, fontSize: 13,
              fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
          Text(s.songArtist, style: const TextStyle(color: AppColors.grey, fontSize: 11,
              fontFamily: 'Roboto')),
        ])),
    ...
    return Container(
      ...
      child: Row(children: [
        Container(width: 36, height: 36,
          decoration: BoxDecoration(color: AppColors.primaryCyan.withValues(alpha: 0.12),
              borderRadius: BorderRadius.circular(9)),
          alignment: Alignment.center,
          child: Text(name.isNotEmpty ? name[0].toUpperCase() : '?',
              style: const TextStyle(color: AppColors.primaryCyan, fontSize: 14,
                  fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        ),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: const TextStyle(color: AppColors.white, fontSize: 13,
              fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
          Text('$st students', style: const TextStyle(color: AppColors.grey,
              fontSize: 11, fontFamily: 'Roboto')),
        ])),
        _iconBtn(Icons.edit_outlined, AppColors.grey,
            () => _showEditClassDialog(index, cls)),
        _iconBtn(Icons.delete_outline, AppColors.errorRed,
  ...
  Widget _activityRow(SessionResult s) {
    ...
    final ago   = diff.inHours < 1 ? '${diff.inMinutes}m ago'
                : diff.inHours < 24 ? '${diff.inHours}h ago' : '${diff.inDays}d ago';
    final score = s.score.round();
    final color = score >= 80 ? const Color(0xFF4CAF50)
                : score >= 60 ? const Color(0xFFFFA726)
                : AppColors.errorRed;
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 11),
      ...
      child: Row(children: [
        Icon(score >= 80 ? Icons.check_circle_outline
            : score >= 60 ? Icons.info_outline : Icons.warning_amber_outlined,
            color: color, size: 17),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Completed "${s.songTitle}"', style: const TextStyle(color: AppColors.white,
              fontSize: 13, fontWeight: FontWeight.w500, fontFamily: 'Roboto')),
          Text('$score% accuracy · $ago', style: const TextStyle(
              color: AppColors.grey, fontSize: 11, fontFamily: 'Roboto')),
        ])),
      ]),
    );
  }
  ...
  Widget _emptyCard(String text, IconData icon) => Container(
    ...
    child: Column(children: [
      ...
      Text(text, style: TextStyle(color: AppColors.grey.withValues(alpha: 0.55),
  ...
  Widget _buildStudentsTab() {
    ...
    for (final cls in _classes) {
      ...
      for (final s in (cls['students'] as List<dynamic>? ?? []).cast<String>()) {
        all.add({'name': s, 'class': n});
      }
    }
    final sections  = ['All Sections', ..._classes.map((c) => c['name'] as String? ?? '')];
    final filtered  = all.where((s) {
      final okSec  = _selectedSection == 'All Sections' || s['class'] == _selectedSection;
      final okSrch = _studentSearch.isEmpty ||
    ...
    return Column(children: [
      ...
      Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            prefixIcon: Icon(Icons.search, color: AppColors.grey.withValues(alpha: 0.5), size: 20),
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: sel ? AppColors.primaryCyan.withValues(alpha: 0.15) : AppColors.inputBg,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                      color: sel ? AppColors.primaryCyan.withValues(alpha: 0.5) : Colors.transparent),
                ),
      ...
      Expanded(
        child: filtered.isEmpty
            ? Center(child: Text(
                all.isEmpty ? 'Add classes and students first'
                    : 'No students match your search',
                style: TextStyle(color: AppColors.grey.withValues(alpha: 0.55),
                    fontSize: 13, fontFamily: 'Roboto')))
            : ListView.builder(
  ...
  Widget _buildStudentCard(Map<String, String> student, int index) {
    ...
    final avg   = _classAvg;
    final flat  = _sessions.isEmpty ? 0.0 : _sessions.map((s) => s.avgFlatPercent).reduce((a,b)=>a+b)/_sessions.length;
    final sharp = _sessions.isEmpty ? 0.0 : _sessions.map((s) => s.avgSharpPercent).reduce((a,b)=>a+b)/_sessions.length;
    ...
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(name, style: const TextStyle(color: AppColors.white, fontSize: 14,
                fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
            Text(cls, style: const TextStyle(color: AppColors.grey, fontSize: 11,
                fontFamily: 'Roboto')),
          ])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${avg.toStringAsFixed(0)}%', style: const TextStyle(
                color: Color(0xFF4CAF50), fontSize: 15,
                fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
            Row(children: [
              const Icon(Icons.trending_up, color: Color(0xFF4CAF50), size: 11),
              const SizedBox(width: 2),
              Text('Improving', style: TextStyle(
        ...
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _miniStat('${_sessions.length}', 'Songs', AppColors.primaryCyan)),
          Container(width: 1, height: 28, color: AppColors.inputBg),
          Expanded(child: _miniStat('${flat.toStringAsFixed(0)}%', 'Flat', const Color(0xFFFFA726))),
          Container(width: 1, height: 28, color: AppColors.inputBg),
          Expanded(child: _miniStat('${sharp.toStringAsFixed(0)}%', 'Sharp', const Color(0xFFEF5350))),
        ]),
        const SizedBox(height: 10),
        ClipRRect(
          borderRadius: BorderRadius.circular(4),
          child: LinearProgressIndicator(
            value: (avg / 100).clamp(0.0, 1.0), minHeight: 4,
  ...
  Widget _buildAssignmentsTab() {
    ...
    for (final cls in _classes) {
      ...
      for (final song in (cls['assignedSongs'] as List<dynamic>? ?? []).cast<String>()) {
        final ss = _sessions.where((s) => s.songTitle == song).toList();
        assignments.add({
          'song': song, 'class': cName, 'students': students,
          'completed': ss.length.clamp(0, students),
          'avgScore': ss.isEmpty ? 0.0 : ss.map((s) => s.score).reduce((a,b)=>a+b)/ss.length,
        });
      }
    }
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: AppColors.primaryCyan,
              borderRadius: BorderRadius.circular(12),
              boxShadow: [BoxShadow(color: AppColors.primaryCyan.withValues(alpha: 0.25),
                  blurRadius: 12, offset: const Offset(0, 4))],
            ),
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: const [
              Icon(Icons.add_rounded, color: Colors.black, size: 20),
              SizedBox(width: 8),
              Text('Create New Assignment', style: TextStyle(color: Colors.black,
      ...
      Expanded(
        child: assignments.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 12),
                Text('No assignments yet', style: TextStyle(
                    color: AppColors.grey.withValues(alpha: 0.6),
                    fontSize: 15, fontFamily: 'Roboto')),
                const SizedBox(height: 6),
                Text('Add songs to a class to get started',
                    style: TextStyle(color: AppColors.grey.withValues(alpha: 0.4),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(song, style: const TextStyle(color: AppColors.white, fontSize: 14,
                fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
            Text(cls, style: const TextStyle(color: AppColors.grey,
                fontSize: 11, fontFamily: 'Roboto')),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Completion Rate', style: TextStyle(color: AppColors.grey,
              fontSize: 12, fontFamily: 'Roboto')),
          Text('$completed/$students (${(pct * 100).round()}%)',
              style: const TextStyle(color: AppColors.grey, fontSize: 12, fontFamily: 'Roboto')),
        ]),
        const SizedBox(height: 6),
        ClipRRect(
          borderRadius: BorderRadius.circular(4),
  ...
  Widget _chip(String text, Color color) => Container(
    ...
    child: Text(text, style: TextStyle(color: color, fontSize: 10, fontFamily: 'Roboto')),
  );
  ...
  Widget _outlineBtn(String label, IconData icon) => Expanded(
    child: Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text(label, style: const TextStyle(color: AppColors.primaryCyan, fontSize: 11,
            fontWeight: FontWeight.w600, fontFamily: 'Roboto')),
      ]),
    ),
  );
  ...
  Widget _buildAnalyticsTab() {
    final avgFlat   = _sessions.isEmpty ? 0.0 : _sessions.map((s) => s.avgFlatPercent).reduce((a,b)=>a+b)/_sessions.length;
    final avgSharp  = _sessions.isEmpty ? 0.0 : _sessions.map((s) => s.avgSharpPercent).reduce((a,b)=>a+b)/_sessions.length;
    final avgSilent = _sessions.isEmpty ? 0.0 : _sessions.map((s) => (1-s.overallVoiceActivity)*100).reduce((a,b)=>a+b)/_sessions.length;
    final weekly    = _weeklyScores();

    final tagPct = _sessions.isEmpty ? 62 : (_sessions.where((s)=>_isTagalog(s.songTitle)).length/_sessions.length*100).round();
    final bisPct = _sessions.isEmpty ? 10 : (_sessions.where((s)=>_isBisaya(s.songTitle)).length/_sessions.length*100).round();
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
        ...
        child: Column(children: [
          ...
          Row(
            ...
            children: List.generate(weekly.length, (i) =>
              Text('W${i+1}', style: TextStyle(
                  color: AppColors.grey.withValues(alpha: 0.5),
                  fontSize: 9, fontFamily: 'Roboto'))),
          ),
          const SizedBox(height: 4),
          Text('Average accuracy over ${weekly.length} weeks',
              style: TextStyle(color: AppColors.grey.withValues(alpha: 0.45),
                  fontSize: 10, fontFamily: 'Roboto'),
      ...
      _analyticsCard(
        ...
        child: Column(children: [
          _issueBar('Flat Notes (High Range)', avgFlat,   const Color(0xFFFFA726)),
          const SizedBox(height: 12),
          _issueBar('Sharp Notes (Sustained)', avgSharp,  AppColors.primaryCyan),
          const SizedBox(height: 12),
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: const [
            Icon(Icons.lightbulb_outline_rounded, color: AppColors.primaryCyan, size: 16),
            SizedBox(width: 7),
            Text('Class Recommendations', style: TextStyle(color: AppColors.primaryCyan,
                fontSize: 15, fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
          ]),
          const SizedBox(height: 12),
          ..._buildRecommendations(avgFlat, avgSharp, avgSilent)
              .map((r) => _recRow(r)),
        ]),
      ),
      const SizedBox(height: 14),
        ...
        _analyticsCard(
          ...
          child: Column(children: [
            ..._bySong().entries.map((e) {
              final avg  = e.value.map((s)=>s.score).reduce((a,b)=>a+b)/e.value.length;
              final flat = e.value.map((s)=>s.avgFlatPercent).reduce((a,b)=>a+b)/e.value.length;
              final shp  = e.value.map((s)=>s.avgSharpPercent).reduce((a,b)=>a+b)/e.value.length;
              return Container(
                margin: const EdgeInsets.only(bottom: 10),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Expanded(child: Text(e.key, style: const TextStyle(color: AppColors.white,
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              ...
              border: Border.all(color: AppColors.inputBg)),
          child: Row(children: [
            const Icon(Icons.copy_outlined, color: AppColors.primaryCyan, size: 16),
            const SizedBox(width: 10),
            const Expanded(child: Text('Export All Reports to Clipboard',
                style: TextStyle(color: AppColors.primaryCyan, fontSize: 13,
                    fontWeight: FontWeight.w600, fontFamily: 'Roboto'))),
            Icon(Icons.chevron_right, color: AppColors.primaryCyan.withValues(alpha: 0.5), size: 18),
          ]),
        ),
      ),
    ]);
  ...
  Widget _analyticsCard({required String title, required IconData icon, required Widget child}) =>
      Container(
        padding: const EdgeInsets.all(16),
        decoration: _card(),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _issueBar(String label, double pct, Color color) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Text(label, style: const TextStyle(color: AppColors.white,
              fontSize: 12, fontFamily: 'Roboto'))),
          Text('${pct.toStringAsFixed(0)}%', style: TextStyle(color: color,
              fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        ]),
        const SizedBox(height: 5),
        ClipRRect(
          borderRadius: BorderRadius.circular(4),
  ...
  Widget _langBox(String pct, String label, Color color) => Expanded(
    child: Container(
      ...
      child: Column(children: [
        Text(pct, style: TextStyle(color: color, fontSize: 20,
            fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        const SizedBox(height: 3),
  ...
  Widget _recRow(String text) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Icon(Icons.check_circle_rounded, color: AppColors.primaryCyan, size: 14),
      const SizedBox(width: 9),
      Expanded(child: Text(text, style: TextStyle(
          color: AppColors.white.withValues(alpha: 0.85),
          fontSize: 12, fontFamily: 'Roboto', height: 1.4))),
    ]),
  );
  ...
  List<String> _buildRecommendations(double flat, double sharp, double silent) {
    ...
    if (sharp  > 15) r.add('Practice slow, sustained phrases to reduce sharpness');
    if (silent > 30) r.add('Schedule breath control workshops to improve voice activity');
  ...
  bool _isTagalog(String t) => ['dadalhin','tala','buwan','ikaw','ako','ang','ng','sa','mahal','puso']
      .any((w) => t.toLowerCase().contains(w));
  bool _isBisaya(String t)  => ['gugma','palangga','bisaya','cebuano','cebu']
      .any((w) => t.toLowerCase().contains(w));

  Map<String, List<SessionResult>> _bySong() {
    final m = <String, List<SessionResult>>{};
    for (final s in _sessions) { m.putIfAbsent(s.songTitle, () => []).add(s); }
    return m;
  }
  ...
  void _showAddClassDialog() {
    ...
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(builder: (ctx, setL) => _dialog(
        ctx: ctx, title: 'New Class',
        content: _classForm(nameCtrl, stuCtrl, students, selSongs, setL),
        onSave: () async {
          if (nameCtrl.text.trim().isEmpty) return;
          await SessionStorageService.addClass({
            'name': nameCtrl.text.trim(), 'students': students, 'assignedSongs': selSongs,
          });
  ...
  void _showEditClassDialog(int index, Map<String, dynamic> cls) {
    ...
    final selSongs = List<String>.from(cls['assignedSongs'] as List<dynamic>? ?? []);
    showDialog(
      context: context,
      builder: (ctx) => StatefulBuilder(builder: (ctx, setL) => _dialog(
        ctx: ctx, title: 'Edit Class',
        content: _classForm(nameCtrl, stuCtrl, students, selSongs, setL),
        onSave: () async {
        ...
        actions: [
          TextButton(
            ...
            child: Text('Cancel', style: TextStyle(
                color: AppColors.grey.withValues(alpha: 0.8), fontFamily: 'Roboto')),
          ),
          ElevatedButton(
            onPressed: onSave,
  ...
  Widget _classForm(TextEditingController nameCtrl, TextEditingController stuCtrl,
      List<String> students, List<String> selSongs, StateSetter setL) =>
      SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          _fLabel('Class Name'), const SizedBox(height: 6),
          _fField(nameCtrl, 'e.g. Grade 9 — Rizal'),
          const SizedBox(height: 14),
          _fLabel('Students'), const SizedBox(height: 6),
          Row(children: [
            ...
            GestureDetector(
              onTap: () {
                ...
                if (n.isNotEmpty) { setL(() => students.add(n)); stuCtrl.clear(); }
              },
              child: Container(
                padding: const EdgeInsets.all(11),
                decoration: BoxDecoration(color: AppColors.primaryCyan,
                    borderRadius: BorderRadius.circular(10)),
              ...
              children: students.map((s) => GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Text(s, style: const TextStyle(color: AppColors.primaryCyan,
                        fontSize: 12, fontFamily: 'Roboto')),
                    const SizedBox(width: 5),
                    const Icon(Icons.close, color: AppColors.primaryCyan, size: 12),
                  ]),
                ),
              )).toList(),
            ),
          ...
          const SizedBox(height: 14),
          _fLabel('Assign Songs'), const SizedBox(height: 6),
          Wrap(spacing: 6, runSpacing: 6,
            children: _availableSongs.map((song) {
              final sel = selSongs.contains(song);
              return GestureDetector(
                onTap: () => setL(() {
                  if (sel) { selSongs.remove(song); } else { selSongs.add(song); }
                }),
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    if (sel)
                      const Padding(padding: EdgeInsets.only(right: 4),
                          child: Icon(Icons.check_rounded, color: AppColors.primaryCyan, size: 12)),
  ...
  Widget _fLabel(String t) => Text(t, style: TextStyle(
      color: AppColors.grey.withValues(alpha: 0.8), fontSize: 11,
      fontWeight: FontWeight.w600, letterSpacing: 0.4, fontFamily: 'Roboto'));
  ...
  Widget _fField(TextEditingController c, String hint) => TextField(
    ...
    decoration: InputDecoration(
      hintText: hint,
      hintStyle: TextStyle(color: AppColors.grey.withValues(alpha: 0.4), fontSize: 12),
      filled: true, fillColor: AppColors.inputBg,
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: const [
          ...
          SizedBox(width: 8),
          Text('Delete Class', style: TextStyle(color: AppColors.white,
              fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
        ]),
        content: const Text('This will permanently remove the class and all its data.',
            style: TextStyle(color: AppColors.grey, fontFamily: 'Roboto', fontSize: 13)),
        actions: [
  ...
  Widget _scoreBadge(int score) {
    final color = score >= 80 ? const Color(0xFF4CAF50)
                : score >= 50 ? const Color(0xFFFFA726)
                : AppColors.errorRed;
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.12),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: color.withValues(alpha: 0.3))),
      child: Text('$score%', style: TextStyle(color: color, fontSize: 12,
          fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
    );
  }
  ...
  List<double> _weeklyScores() {
    ...
    return List.generate(8, (i) {
      ...
      final ws = _sessions.where((s) => s.completedAt.isAfter(wS) && s.completedAt.isBefore(wE)).toList();
      return ws.isEmpty ? 0.0 : ws.map((s)=>s.score).reduce((a,b)=>a+b)/ws.length;
    });
  }
}
...
class _TrendPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Grid lines
    final gridP = Paint()..color = AppColors.inputBg..strokeWidth = 1;
    for (int i = 0; i <= 4; i++) {
      final y = h - (i / 4) * h;
      canvas.drawLine(Offset(0, y), Offset(w, y), gridP);
      final tp = TextPainter(
        text: TextSpan(text: '${i * 25}%',
            style: TextStyle(color: AppColors.grey.withValues(alpha: 0.4),
    ...
    canvas.drawPath(fill, Paint()
      ..shader = LinearGradient(
        colors: [AppColors.primaryCyan.withValues(alpha: 0.25),
